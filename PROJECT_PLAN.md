...
- [ ] listing_parking + формы
- [ ] listing_land + формы
- [ ] listing_commercial + формы
- [ ] listing_houses + формы
- [ ] Обновление фильтров на фронте под новые типы

### Phase 5 — Расширение регионов

- [ ] Согласовать доступ к фидам СПБ, КРД, НСК и др. через менеджера ТА
- [ ] Добавить регионы в `feed_regions`
- [ ] Скрипт загрузки → обобщить по `TRENDAGENT_REGION`
...
- [ ] Модуль requests: POST + Telegram-уведомление *(POST `/requests` и админка; уведомления в Telegram не подключены)*
- [ ] Модуль favorites: CRUD + sync *(API `GET/POST/DELETE /favorites`; публичный UI пока на localStorage, не синхронизация с API)*
...
- [ ] Избранное / Сравнение *(страницы есть; данные не подгружаются из API, избранное без обязательной авторизации)*
...
- [ ] Каталоги: дома, участки, коммерция (Phase 4 из исходного плана)
...
- [ ] Блок «Горячие предложения» — с бейджами *(секция есть; данные пока моковые)*
- [ ] Блок «Старт продаж» — реальные даты *(секция есть; данные пока моковые)*
- [ ] Форма «Подобрать объект» → заявка в БД + Telegram *(заявка уходит в БД через `POST /requests`; Telegram — нет)*
- [x] Блок «О платформе»
- [x] Блок «Инструменты» — 4 карточки
- [ ] Блок «Последние новости» — из парсера *(лента из `GET /news`; авто-парсер TG/RSS — не подключён)*
...
- [ ] Пагинация: 20 объектов, «Показать ещё» *(постранично через «Назад/Вперёд», другой размер страницы)*
- [x] Счётчик «N объектов»
- [ ] Сортировка: по цене / площади / названию *(в списке ЖК — нет; на странице ЖК сортировка таблицы квартир есть)*
...
- [ ] Кнопки: презентация, избранное, сравнение *(кнопки в UI; полный сценарий с API/презентацией — частично)*
- [ ] Форма заявки → БД + Telegram *(компонент формы без `POST /requests`; Telegram — нет)*

**Страница квартиры (`/apartment/:id`):**
- [x] Фото + планировка *(по данным API / fallback)*
- [x] Все параметры *(основной набор полей из API)*
- [x] Ссылка на ЖК
- [ ] Кнопки: заявка, избранное, презентация *(UI; заявка через общий LeadForm не в БД)*
...
- [ ] Список справа / снизу на мобиле *(упор на карту и фильтры; отдельная колонка списка как в ТЗ — уточняется)*

**Ипотека (`/mortgage`):**
- [x] Калькулятор: стоимость, взнос, срок, ставка → платёж
- [ ] Форма заявки → БД + Telegram *(LeadForm — заглушка без API; Telegram — нет)*

**Сравнение (`/compare`):**
- [x] До 3 объектов *(лимит в UI)*
- [ ] Параметры в строках *(места под данные из API — нет)*
- [ ] Удаление / добавление в избранное *(удаление из списка сравнения есть; связка с карточками/избранным — нет)*

**Избранное (`/favorites`):**
- [ ] Требует авторизации *(TODO в коде; не редиректит на `/login`)*
- [ ] Карточки + удаление *(удаление id из localStorage; карточки без загрузки из API)*
- [ ] «Скачать подборку» *(кнопка вызывает `window.print()`, не выгрузка PDF/подборки)*

**Авторизация:**
- [ ] Login: телефон + пароль *(сейчас email + пароль к JWT)*
- [ ] Telegram Login Widget *(кнопка-заглушка / TODO)*
- [ ] Register: имя, телефон, email, пароль *(есть имя, email, пароль; телефон и отправка на API регистрации — нет)*
- [ ] Профиль: данные, заявки, избранное, подборки *(страница-заглушка без связи с API)*
...
| Область | Статус |
|--------|--------|
| Auth: Telegram Login, телефон в login/register | Не готово |
| Заявки → Telegram | Не готово |
...
| Главная: «Горячие» / «Старт продаж» из API (не моки) | Частично / уточнить приёмку |
| Пагинация каталога ЖК «20 + показать ещё» | Другая схема |
| Сортировка списка ЖК | Нет |
| Каталоги дома / участки / коммерция (Sprint 4) | Не готово |
| Ручной ввод объектов (квартиры, дома, участки, коммерция) вне фида | **Не готово** — требуется отдельный этап: формы админки, API `listing` create/update, привязка к региону/ЖК, валидация. Регионы и справочник регионов для таких объектов уже не блокируются только фидом. |
| Белгород: ручной импорт контента | Страница есть; контур импорта — нет |
| Парсер новостей TG/RSS | Не готово |
