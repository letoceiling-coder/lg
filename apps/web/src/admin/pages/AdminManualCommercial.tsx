...
type ListingCommercial = {
  commercialType: 'OFFICE' | 'RETAIL' | 'WAREHOUSE' | 'RESTAURANT' | 'OTHER' | null;
...
const commercialTypeOptions = [
  ...
  { value: 'RESTAURANT', label: 'Ресторан/общепит' },
...
export default function AdminManualCommercial() {
  ...
  const { data: current, isLoading: loadingCurrent } = useQuery({
    queryKey: ['admin', 'manual-commercial', editId],
  ...
  const { data: blocksData } = useQuery({
    queryKey: ['blocks', 'for-manual-commercial', effectiveRegionId],
    queryFn: () =>
  ...
  useEffect(() => {
    ...
    setHasSeparateEntrance(Boolean(current.commercial?.hasSeparateEntrance));
    setDidInitForm(true);
  ...
  const mutation = useMutation({
    mutationFn: async () => {
      ...
      return apiPost('/admin/listings/manual-commercial', body);
    },
    onSuccess: () => {
      toast.success(isEdit ? 'Коммерческий объект обновлён' : 'Коммерческий объект создан');
    ...
    <div className="p-6 max-w-3xl space-y-4">
      <div className="flex items-center gap-2">
        ...
        <h1 className="text-2xl font-bold">
          {isEdit ? 'Редактирование коммерции' : 'Новый коммерческий объект (MANUAL)'}
        </h1>
      </div>
      ...
      <div className="rounded-xl border p-4 grid grid-cols-1 md:grid-cols-2 gap-3">
        ...
        <label className="flex items-center gap-2 text-sm">
          <input
            ...
            onChange={(e) => setHasSeparateEntrance(e.target.checked)}
          />
          Отдельный вход
        </label>
        <label className="flex items-center gap-2 text-sm">
